...
% shown in Figure 4.6 of the thesis.

gamma = 1/pi;

r1 = 0; % inner radius of the nonscattering region
r2 = linspace(0,1,11); % possible values for the outer radius of the nonscattering region
...
g1s = zeros(1,11); % vector for storing g1(s) for each r2
g2s = zeros(1,11); % vector for storing g2(s) for each r2
g3s = zeros(1,11); % vector for storing g3(s) for each r2
a1s = zeros(1,11); % vector for storing a1(s) for each r2
a2s = zeros(1,11); % vector for storing a2(s) for each r2
a3s = zeros(1,11); % vector for storing a3(s) for each r2
a4s = zeros(1,11); % vector for storing a3(s) for each r2
...
for i=1:11 % a step for computing a1,a2,a3,a4 corresponding to each r2
    fun12 = @(theta) (0.5*(r1^2*cos(theta)-r1*r2(i)*(1+cos(theta).^2) + r2(i)^2*cos(theta))./(r1^2 - 2*r1*r2(i)*cos(theta) + r2(i)^2).^(3/2) ).*exp(-muan*(r1^2 - 2*r1*r2(i)*cos(theta) + r2(i)^2).^(1/2));
    fun3 = @(theta) (((1 - cos(theta)).^(1/2))).*exp(-muan*r2(i)*(2*(1-cos(theta))).^(1/2));
...
Z = zeros(11,3); % A matrix for storing the coefficients for the possible values for the inner radius
...
for i=1:11
    ...
    A(2,1) = 2*gamma*a2s(i)*besseli(0,delta*r2(i));
    A(2,2) = 2*gamma*a2s(i)*besselk(0,delta*r2(i));
    A(2,3) = 2*gamma*a1s(i)*besseli(0,delta*r1) + K*delta*besseli(1,delta*r1);
    
    % The third row is (4.19). As you noted, r1 should be r2 at one point in
    % (4.19):
    A(3,1) = 2*gamma*a4s(i)*besseli(0,delta*r2(i)) - K*delta*besseli(1,delta*r2(i));
    A(3,2) = 2*gamma*a4s(i)*besselk(0,delta*r2(i)) + K*delta*besselk(1,delta*r2(i));
    A(3,3) = 2*gamma*a3s(i)*besseli(0,delta*r1);
